...
class PayrollRegularos(models.Model):
    _name = 'payroll.regularos'
    ...
    line_ids = fields.One2many('payroll.regularos.line', 'payrollregularos_id', 'Employee', readonly=True, states={'draft': [('readonly', False)],})

    @api.model
    def create(self, vals):
        # assigning the sequence for the record
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('payroll.regularos') or _('New')
        res = super(PayrollRegularos, self).create(vals)
        return res
...
class PayrollRegularosLine(models.Model):
    ...
    payrollregularos_id = fields.Many2one('payroll.regularos', 'Payroll',)
